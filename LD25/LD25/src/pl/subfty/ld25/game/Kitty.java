...
public class Kitty extends Group{
	...
	private static float RADIUS = 0.85f;
  ...
  //LOVING
	private class LovePool extends Pool<SpriteActor>{
		...
		protected SpriteActor newObject() {
			SpriteActor s = new SpriteActor(SPRITES.LOVE, 0.475f);
  ...
  //BOX2D STUFF
	...
	public void restart(LEVEL_DEF level){
		bgImg.setSprite(SPRITES.CAT,RADIUS*2*1.2f);
		...
		body.setTransform(this.x = (630)*SC, 
						  this.y = (1024-285)*SC, 0); 	
	}
	  ...
	  //THERE IS NO GOING BACK WHEN STARTED TO MAKE POO
		if(pooMeter == POO_METER_MAX){
			...
			if(maxPooW8 > POO_COMMITING_TIME){
				poo.makePoo(this.x+this.width/2, this.y+this.height/2);
				pooCommited = true;
				pooMeter=0;
				maxPooW8=0;
				addWX = addWY = 0;
