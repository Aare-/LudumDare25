...
public class BFonts extends BitmapFont{
  ...
  //DRAWING WITH ALPHA
	public TextBounds drawMultiLine (SpriteBatch spriteBatch, CharSequence str, float x, float y, float alpha, boolean smooth, boolean front) {
		return drawMultiLine(spriteBatch, str, x, y, 0, HAlignment.LEFT, alpha, smooth, front);
	}
	public TextBounds drawMultiLine (SpriteBatch spriteBatch, CharSequence str, float x, float y, float alignmentWidth,
			HAlignment alignment, float alpha, boolean smooth, boolean front) {
		float batchColor = spriteBatch.getColor().toFloatBits();
		float down = this.data.down;
		int start = 0;
		int numLines = 0;
		...
		while (start < length) {
			...
			float lineWidth = draw(spriteBatch, str, x + xOffset, y, start, lineEnd, alpha, smooth, front).width;
			maxWidth = Math.max(maxWidth, lineWidth);
			start = lineEnd + 1;
			y += down;
			numLines++;
	...
	float startS,
		  initA;
	public TextBounds draw (SpriteBatch spriteBatch, CharSequence str, float x, float y, int start, int end, float alpha, boolean smooth, boolean front) {
		initA = actColor.a;
		...
		if (data.scaleX == 1 && data.scaleY == 1) {
			...
			while (start < end) {
				...
				if (lastGlyph != null) {
					...
					actColor.a = Math.max(0, Math.min(1, ((front ? 1 : 0)-(start)/(strLen)+alpha)));
			...
			while (start < end) {
				...
				actColor.a = Math.max(0, Math.min(1, ((front ? 1-(start)/(strLen)+alpha : (start)/(strLen)+alpha))));
				if(!smooth && actColor.a < 1)
					actColor.a = 0;
				actColor.a *= initA;
